...
-- ยุบรวมการ SUM(pmdo033) ในระดับ pmdldocno / seq ก่อน JOIN
LEFT JOIN (
    SELECT pmdodocno, pmdoseq, pmdo011, pmdo012, SUM(pmdo033) AS total_pmdo033
    FROM pmdo_t
    WHERE pmdoent = '666'
    GROUP BY pmdodocno, pmdoseq, pmdo011, pmdo012
) c ON c.pmdodocno = b.pmdldocno

LEFT JOIN (
    SELECT pmdbdocno, pmdb004, SUM(pmdb006) AS total_pmdb006
    FROM pmdb_t
    WHERE pmdbent = '666'
...
-- ตัดการ JOIN apce_t ซ้ำซ้อนออก เหลือ g เพียงตัวเดียว
LEFT JOIN apce_t g
